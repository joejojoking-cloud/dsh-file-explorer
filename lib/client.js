		...
		const highlight = (text, lang) => {
			...
			while (i < n) {
				if (multi) {
					...
					if (j === -1) { html += hlSpan(multi.cls, escapeHtml(src.slice(i))); i = n; break }
					html += hlSpan(multi.cls, escapeHtml(src.slice(i, j + multi.close.length)));
				...
				if (cfg.tr && (src.slice(i, i + 3) === '"""' || src.slice(i, i + 3) === "'''")) {
					multi = { cls: 's', close: src.slice(i, i + 3) };
					i += 3; continue;
				}
				if (cfg.de && c === '@') {
